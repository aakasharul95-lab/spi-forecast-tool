...
def run_simulation(total_ih, se_count, ih_per_se, ramp_wks,
    ...
    for i, wk in enumerate(weeks):
        ...
        cap       = full_cap * ramp
        remaining = max(0.0, total_ih - cum)   # FIX: cap sent by remaining scope only
        avail     = pool[i] + bl
...
for name in ("RG", "SOP", "EG"):
    ...
    label = f"{name}: {s['done']:,}/{total_ih:,} IH\n{'✓ On track' if s['ok'] else f'✗ Miss {s[\"miss\"]:,} IH'}"
...
# SE capacity (amber dash-dot)
ax2.plot(df["idx"], df["cap"], color=CLR_CAPACITY, lw=1.8, ls="-.",
         zorder=3, alpha=0.85, label="SE capacity")
